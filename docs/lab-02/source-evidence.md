...
| **Annual Sequence Reset** | SDS p. 2 (D-10) | **Specified** | The sequence counter resets to 1 annually on January 1st. | Query checks `WHERE ticketNo LIKE 'TKT-' || currentYear || '-%'` to determine next sequence. |
...
| **Attachment Soft Removal** | SDS p. 7, 14-15; Labsheet Sec. 4.5 | **Specified** | Removal is soft removal: row retained as tombstone (`deletedAt`, `deletedById`). Physical binary deleted immediately (D-11). Requires confirmation and reason. | Endpoint `POST /api/v1/tickets/:id/attachments/:attachmentId/remove` requires `{ reason: string }`, sets `deletedAt/deletedById`, unlinks file. |
...
| **Download Block on Removed** | Labsheet Sec. 4.5; SDS p. 14 | **Specified** | Removed attachments cannot be downloaded or previewed. Metadata remains visible in audit log. | `GET .../attachments/:attachmentId` returns 404/410 if `deletedAt !== null`. |
